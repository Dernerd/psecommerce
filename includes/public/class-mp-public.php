...
//don't verify ssl connections
			...
			if ( is_wp_error( $tmp ) ) {
				@unlink( $tmp );
				trigger_error( "PSeCommerce was unable to download the file $url for serving as download: " . $tmp->get_error_message(), E_USER_WARNING );
...
//partial download headers
			...
			if ( $filesize > $chunksize ) {
				...
				if ( $handle === false ) {
					trigger_error( "PSeCommerce was unable to read the file $tmp for serving as download.", E_USER_WARNING );
